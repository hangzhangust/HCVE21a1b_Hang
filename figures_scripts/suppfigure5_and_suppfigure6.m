...
for i = 1:length(B1)
    for j =1:length(ind_non_conserve)
        ...
        Mutant_MSA(i,j) = sum(Peak_MSA(i,Start_site:End_site));
    end
end
...
load('Model_1b.mat')
load('peak_pos50_1b.mat', 'Peaks')
beta_MSA = 1+length(B1)*sum(log(B1./min(B1))).^-1
...
set(get(gca,'YLabel'), 'Units', 'Normalized', 'Position', [-0.27, 0.53, 0]);
...
set(gca,'TickDir','out')
set(gca,'TickLength',[0.035, 0.03])
h=legend('Subtype 1a', 'Subtype 1b','Location','best');
legend boxoff
set(h,...
    'Position',[0.13555659656467 0.76319043784449 0.543248951686586 0.188492067276485]);
...
set(gca,'Position',[.21 .25 .73 .71]);  %调整 XLABLE和YLABLE不会被切掉
% set(gcf,'Position',[10 10 7.84 6]);
% set(gca,'Position',[.2 .2 .78 .74]);  %调整 XLABLE和YLABLE不会被切掉
figure_FontSize=8;
set(get(gca,'XLabel'),'FontSize',figure_FontSize,'Vertical','top');
