function DI=dca_Ahmed_W(inputfile, outputfile,pseudocount_weight,W)

%takes weight calculated based on number of patients as input instead of
%theta

% Direct Coupling Analysis (DCA) -- modified to take weight vector as input
%
...
%   pseudocount_weight - pseudocount value (suggested value = 0.5)
%   W          - vector including weight of each sample in the inputfile
%
...
%     pseudocount_weight = 0.5; 2/(757+1); 0.0066; % relative weight of pseudo count   
%     theta = 0.2;              % threshold for sequence id in reweighting

    
    [N,M,q,align] = return_alignment(inputfile); 
    [Pij_true,Pi_true, Meff]=Compute_True_Frequencies(align,M,N,q, W);
    ...
    DI=Compute_Results(Pij, Pi,Pij_true, Pi_true, invC, N, q,fp);
    fclose(fp); 
end
...
function DI=Compute_Results(Pij,Pi,Pij_true,Pi_true,invC, N,q, fp)
...
function [Pij_true,Pi_true,Meff] = Compute_True_Frequencies(align,M,N,q,W)
